...
def parse_hook_bump_rust_dep(ec, _):
    """
    Bump Rust build dependency to a newer version that is compatible with OpenSSL 3.0.x (only in EESSI 2023.04)
    """
    rust_version_bumps = {
        '1.52.1': '1.60.0',  # for 2021a generation (GCCcore/10.3.0)
        '1.54.0': '1.56.0',  # for 2021b generation (GCCcore/11.2.0)
    }
    eessi_version = get_eessi_envvar('EESSI_PILOT_VERSION') 
    if LooseVersion(eessi_version) >= LooseVersion('2023.04'):
        build_deps = ec['builddependencies']
        for idx, build_dep in enumerate(build_deps):
            if build_dep[0] == 'Rust':
                rust_ver = build_dep[1]
                new_rust_ver = rust_version_bumps.get(rust_ver)
                if new_rust_ver is not None:
                    build_deps[idx] = ('Rust', new_rust_ver)
                    msg = f"Version of Rust build dependency for {ec.name} v{ec.version} "
                    msg += f"({ec.toolchain.mod_short_name}) bumped from {rust_ver} to {new_rust_ver}"
                    print_msg(msg, log=ec.log)
...
PARSE_HOOKS = {
    ...
    'Python': parse_hook_bump_rust_dep,
